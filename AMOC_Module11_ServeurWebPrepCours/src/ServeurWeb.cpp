...
ServeurWeb::ServeurWeb() {
  ...
  m_webServer->on("/actionneur", HTTP_GET, [this]() { this->actionneur(); });

  m_webServer->on("/actionneurs/1", HTTP_PUT, [this]() { this->modifierActionneur(); });
...
void ServeurWeb::actionneur() {
  String etat = (digitalRead(LED_BUILTIN) == HIGH) ? "allume" : "eteint";
  String json = "{\"id\":1,\"etat\":\"" + etat + "\"}";
  m_webServer->send(200, "application/json", json);
}

void ServeurWeb::modifierActionneur() {
  String body = m_webServer->arg("plain");

  if (body.indexOf("\"etat\":\"allume\"") != -1) {
    digitalWrite(LED_BUILTIN, HIGH);
  } else if (body.indexOf("\"etat\":\"eteint\"") != -1) {
    digitalWrite(LED_BUILTIN, LOW);
  } else {
  ...
  String nouvelEtat = (digitalRead(LED_BUILTIN) == HIGH) ? "allume" : "eteint";
  String json = "{\"id\":1,\"etat\":\"" + nouvelEtat + "\"}";
  m_webServer->send(200, "application/json", json);
}